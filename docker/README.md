...
```sh
docker run -it -e postgres_host=host.docker.internal -e postgres_port=5432 -e postgres_user=graph-node -e postgres_pass=oh-hello -e postgres_db=graph-node -e ipfs=host.docker.internal:5001 -e ethereum=mainnet:https://sphinx.shardeum.org graphprotocol/graph-node:latest
```
